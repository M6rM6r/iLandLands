...
/**
 * Immutable Data Object for Listing Context
 */
...
class OpenAiService
{
  ...
  public function generatePropertyDescription(array $listing): string
  {
    ...
    $ctx = new ListingContext(
      (string) ($listing['id'] ?? ''),
      (string) ($listing['tenant_id'] ?? ''),
      (string) ($listing['title'] ?? 'Land'),
      (float) ($listing['area'] ?? 0),
      (float) ($listing['price'] ?? 0),
      (string) ($listing['country'] ?? '')
    );

    if ($ctx->tenantId === '' || $ctx->id === '')
      return '';

    $cacheKey = 'property_desc_' . $ctx->id;

    // 1. Cache hit
    $cached = $this->getCached($ctx->tenantId, $cacheKey);
    if ($cached !== null) {
      return $cached;
    }
    ...
    // 4. Persist to cache
    $this->saveCache($ctx->tenantId, $cacheKey, $description);
  ...
  private function buildPrompt(ListingContext $ctx): string
  {
    $title = $ctx->title;
    $area = $ctx->area;
    $price = $ctx->price;
    $country = $ctx->country;
    $location = 'N/A'; // Assuming location is not in ListingContext yet, or needs to be passed separately
...
PROMPT;
   ...
   * Refactored to use more professional system instructions inspired by ISL270's 
   * real-estate-ai-flutter-app.
   */
