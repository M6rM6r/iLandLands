<?php
/**
 * PayTabsService — alternative Gulf payment gateway integration.
 *
 * Ported from Rew (realestate-saas) lib/billing/paytabs.ts.
 *
 * PayTabs supports SAR, AED, QAR, BHD, OMR, KWD (full Gulf coverage).
 * Uses the PayTabs Hosted Payment Page flow (PT2 API).
 *
 * Environment variables required:
 *   PAYTABS_SERVER_KEY — API server key from PayTabs merchant dashboard
 *   PAYTABS_PROFILE_ID — Payment page profile ID
 *   PAYTABS_REGION     — PayTabs regional endpoint code: SAU|UAE|QAT|BHR|OMN|KWT|EGY (default: UAE)
 *   APP_BASE_URL       — e.g. https://api.gulflands.com
 *
 * @see https://support.paytabs.com/en/support/solutions/articles/60000712929
 */
class PayTabsService
{
    ...
    /**
     * Create a hosted payment page and return the checkout URL.
     *
     * @param array{
     *     cart_id: string,
     *     amount: float,
    ...
    public function createCheckout(array $params): array
    {
        ...
        $currency   = strtoupper($params['currency'] ?? 'AED');
        $description = substr($params['description'] ?? 'Gulf Lands Payment', 0, 100);
    ...
    /**
     * Parse a PayTabs callback/IPN payload into a normalised status struct.
     *
    ...
    public function parseCallback(array $payload): array
    {
        ...
        return [
            ...
            'tranRef'         => $tranRef,
            'cartId'          => $cartId,
            'billingStatus'   => $this->normalizeBillingStatus($responseStatus),
        ];
    }

    /**
     * Normalise any billing status value to one of: paid|pending|failed|unpaid.
     */
    public function normalizeBillingStatus(mixed $value): string
    {
        $v = strtolower(trim((string) $value));
        // PayTabs uses 'A' for approved, 'D' for declined, 'E' for error
    ...
    /**
     * POST JSON to the PayTabs API endpoint.
     *
     * @param array $payload
     * @return array|null  Decoded JSON response, or null on cURL failure.
     */
    ...
    /**
     * Recursively search a nested response object for the checkout URL.
     * Ported from Rew's findNestedUrl() — handles any PayTabs response shape.
     *
     * @param mixed $input
     * @param int   $depth
     * @return string  URL string, or '' if not found.
     */
