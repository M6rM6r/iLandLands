...
class AnalyticsEngine:
        ...
        try:
            async with self.db.get_connection() as conn:
                await conn.execute(
                    ...
                    json.dumps(event.properties), event.user_agent, event.ip_address,
                    event.timestamp
                )
    ...
    async def get_user_behavior(self, user_id: str, days: int = 30) -> UserBehavior:
        ...
        try:
            async with self.db.get_connection() as conn:
                ...
                pages_viewed = len([e for e in events if e['event_name'] == 'listing_viewed'])
                searches_performed = len([e for e in events if e['event_name'] == 'search_performed'])
                favorites_added = len([e for e in events if e['event_name'] == 'added_to_favorites'])
                contact_requests = len([e for e in events if e['event_name'] == 'contact_requested'])
    ...
    async def get_market_insights(self, country: Optional[str] = None) -> List[MarketInsight]:
    ...
    def _prepare_training_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Select features
        feature_columns = ['area'] + [f'country_{c}' for c in countries] + ['is_featured']
        df = df[['price'] + feature_columns]
        
        return df.dropna()

    async def _analyze_price_trends(self, country: Optional[str]) -> Optional[MarketInsight]:
        ...
        try:
            async with self.db.get_connection() as conn:
                ...
                return MarketInsight(
                    ...
                    trend=trend,
                    confidence=0.85,
                    recommendations=self._generate_price_recommendations(trend, change_percentage)
                )
    ...
    async def _analyze_demand_trends(self, country: Optional[str]) -> Optional[MarketInsight]:
        ...
        try:
            async with self.db.get_connection() as conn:
                ...
                return MarketInsight(
                    ...
                    trend=trend,
                    confidence=0.75,
                    recommendations=self._generate_demand_recommendations(trend, change_percentage)
                )
    ...
    async def _analyze_popular_locations(self, country: Optional[str]) -> Optional[MarketInsight]:
    ...
    async def _analyze_user_engagement(self, country: Optional[str]) -> Optional[MarketInsight]:
    ...
    def _generate_price_recommendations(self, trend: str, change_percentage: float) -> List[str]:
        """Generate price-related recommendations"""
        recommendations = []
        
        if trend == "increasing":
            recommendations.append("Market is heating up - consider listing at premium prices")
            recommendations.append("Buyers are active - good time to sell")
        else:
            recommendations.append("Prices are softening - consider competitive pricing")
            recommendations.append("Focus on value proposition to attract buyers")
        
        return recommendations

    def _generate_demand_recommendations(self, trend: str, change_percentage: float) -> List[str]:
...
async def startup_event():
    ...
    # Initialize database
    db_manager = DatabaseManager("postgresql://gulflands_user:secure_password_123@mysql/gulflands")
    await db_manager.initialize()
    
    # Initialize Redis
    redis_manager = RedisManager("redis://:redis_password_123@redis:6379/0")
