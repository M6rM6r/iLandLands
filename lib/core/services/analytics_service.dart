...
abstract class AnalyticsService {
  ...
  Future<void> trackListingView(String listingId, Map<String, dynamic> listingData);
  ...
  Future<void> trackNetworkRequest(String url, int statusCode, Duration duration);
...
class AnalyticsServiceImpl implements AnalyticsService {
  ...
  Future<void> trackEvent(String name, {Map<String, dynamic>? parameters}) async {
    try {
      final enrichedParameters = await _enrichParameters(parameters);
  ...
  Future<void> trackScreenView(String screenName, {Map<String, dynamic>? parameters}) async {
    try {
      ...
      await _trackLocalEvent('screen_view', {
  ...
  Future<void> trackPerformance(String operation, Duration duration) async {
    try {
      await _trackLocalEvent('performance', {
        'operation': operation,
        'duration_ms': duration.inMilliseconds,
      });
      
      logger.d('Performance tracked: $operation took ${duration.inMilliseconds}ms');
    } catch (e) {
  ...
  Future<void> trackSearch(String query, int resultsCount) async {
    await trackEvent(AppConfig.eventSearchPerformed, parameters: {
      'query': query,
  ...
  Future<void> trackFilter(Map<String, dynamic> filters) async {
    await trackEvent(AppConfig.eventFilterApplied, parameters: {
  ...
  @override
  Future<void> trackListingView(String listingId, Map<String, dynamic> listingData) async {
    await trackEvent(AppConfig.eventListingViewed, parameters: {
      'listing_id': listingId,
      AppConfig.propertyCountry: listingData['country'],
      'price_range': _getPriceRange((listingData['price'] as num?)?.toDouble() ?? 0.0),
      'area_range': _getAreaRange((listingData['area'] as num?)?.toDouble() ?? 0.0),
  ...
  Future<void> trackContact(String listingId, String method) async {
    await trackEvent(AppConfig.eventContactRequested, parameters: {
      'listing_id': listingId,
  ...
  Future<void> trackAppLifecycle(String state) async {
    await trackEvent('app_lifecycle', parameters: {'state': state});
  ...
  @override
  Future<void> trackNetworkRequest(String url, int statusCode, Duration duration) async {
  ...
  Future<void> trackConversion(String type, Map<String, dynamic> data) async {
    await trackEvent('conversion', parameters: {
  ...
  Future<void> trackRetention(String cohort, Duration sessionDuration) async {
    await trackEvent('retention', parameters: {
      'cohort': cohort,
  ...
  @override
  Future<void> trackMonetization(String event, double value, String currency) async {
    await trackEvent('monetization', parameters: {
      'event': event,
      'value': value,
      'currency': currency,
    });
  ...
  @override
  Future<void> trackCustomEvent(String name, Map<String, dynamic> parameters) async {
  ...
  Future<Map<String, dynamic>> _enrichParameters(Map<String, dynamic>? parameters) async {
  ...
  Future<void> _trackLocalEvent(String name, Map<String, dynamic> parameters) async {
    final events = sharedPreferences.getStringList('local_events') ?? [];
  ...
  Future<void> _setUserProperties() async {
    final keys = sharedPreferences.getKeys().where((key) => key.startsWith('user_property_'));
    
    for (final key in keys) {
      final value = sharedPreferences.getString(key);
      if (value != null) {
        final propertyName = key.replaceFirst('user_property_', '');
        _userProperties[propertyName] = value;
        await _firebaseAnalytics.setUserProperty(name: propertyName, value: value);
      }
    }
  }
  ...
  Future<void> _trackAppOpen() async {
    ...
    final lastOpen = sharedPreferences.getString('last_open');
    if (lastOpen != null) {
      final lastOpenDate = DateTime.parse(lastOpen);
      final daysSinceLastOpen = DateTime.now().difference(lastOpenDate).inDays;
      
      await trackEvent('app_return', parameters: {
        'days_since_last_open': daysSinceLastOpen,
      });
    }
    
    await sharedPreferences.setString('last_open', DateTime.now().toIso8601String());
  }
